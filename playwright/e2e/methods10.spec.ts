...
test.describe('Methods 10 - refresh() method', () => {
  test('adding new options with disabled or selected in select drop', async ({ page }) => {
    ...
    await expect(await page.locator('.ms-drop li label input[type=checkbox]')).toHaveCount(3);
    await expect(await page.locator('.ms-parent .ms-choice span')).toHaveText('');
    ...
    await page.locator('.ms-parent').click();
    await expect(await page.locator('.ms-drop li label input[type=checkbox]')).toHaveCount(4);
    let selectedLoc = await page.locator('.ms-drop li.selected');
    await expect(selectedLoc).toHaveCount(1);
    await expect(await selectedLoc.locator('label')).toHaveClass('disabled');
    await expect(await page.locator('.ms-parent .ms-choice span')).toHaveText('new disabled option');
    ...
    await expect(await page.locator('.ms-parent .ms-choice span')).toHaveText('new disabled option');
    await page.locator('.ms-parent').click();
    await expect(await page.locator('.ms-drop li label input[type=checkbox]')).toHaveCount(5);
