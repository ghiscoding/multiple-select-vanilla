...
test.describe('Example 04 - Select Auto-Width', () => {
  test('every select drop parent should have different width', async ({ page }) => {
    ...
    assertDropWidth(page, '[data-test=select1].ms-parent', 92);
    await page.getByRole('button', { name: 'First', exact: true }).click();
    await page.locator('[data-test=select1].ms-drop li label').filter({ hasText: 'Fourth' }).click();
    await expect(page.locator('div[data-test=select1] .ms-choice span')).toHaveText('Fourth');

    assertDropWidth(page, 'div[data-test=select2].ms-parent', 108);
    await page.getByRole('button', { name: 'Options 1' }).click();
    await page.locator('[data-test=select2].ms-drop li label').filter({ hasText: 'Options 3' }).click();
    ...
    assertDropWidth(page, 'div[data-test=select6].ms-parent', 170);
