...
test.describe('Options 17 - Drop Container', () => {
  test('first select should not be visible without a container but all other selects should be visible', async ({ page }) => {
    ...
    let drop1elm = await page.locator('.select1.ms-parent .ms-drop');
    ...
    let drop2elm = await page.locator('[data-test=select2].ms-drop');
    ...
    let drop3elm = await page.locator('[data-test=select3].ms-drop');
    ...
    let drop4elm = await page.locator('[data-test=select4].ms-drop');
